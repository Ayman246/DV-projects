module axi4 #(
    ...
    always @(posedge axiIF.ACLK or negedge axiIF.ARESETn) begin
        if (!axiIF.ARESETn) begin
            // Reset all outputs
            axiIF.AWREADY <= 1'b1;  // Ready to accept address
            axiIF.WREADY <= 1'b0;
            axiIF.BVALID <= 1'b0;
            axiIF.BRESP <= 2'b00;
            
            axiIF.ARREADY <= 1'b1;  // Ready to accept address
            axiIF.RVALID <= 1'b0;
            axiIF.RRESP <= 2'b00;
            axiIF.RDATA <= {DATA_WIDTH{1'b0}};
            axiIF.RLAST <= 1'b0;
            ...
            case (write_state)
                W_IDLE: begin
                    ...
                    if (axiIF.AWVALID && axiIF.AWREADY) begin
                        ...
                        write_burst_len <= axiIF.AWLEN;
                        write_burst_cnt <= axiIF.AWLEN;
                        write_size <= axiIF.AWSIZE;
                ...
                W_DATA: begin
                    if (axiIF.WVALID && axiIF.WREADY) begin
                        ...
                        if (write_addr_valid && !write_boundary_cross) begin
                            ...
                            mem_wdata <= axiIF.WDATA;
                        end
                        
                        // Check for last transfer
                        if (axiIF.WLAST || write_burst_cnt == 0) begin
                            axiIF.WREADY <= 1'b0;
                            write_state <= W_RESP;
                            
                            // Set response - delayed until write completion
                            if (!write_addr_valid || write_boundary_cross) begin
                                axiIF.BRESP <= 2'b10;  // SLVERR
                            end else begin
                                axiIF.BRESP <= 2'b00;  // OKAY
                            end
                            axiIF.BVALID <= 1'b1;
                        end else begin
            ...
            case (read_state)
                R_IDLE: begin
                    ...
                    if (axiIF.ARVALID && axiIF.ARREADY) begin
                        // Capture address phase information
                        read_addr <= axiIF.ARADDR;
                        read_burst_len <= axiIF.ARLEN;
                        read_burst_cnt <= axiIF.ARLEN;
                        read_size <= axiIF.ARSIZE;
                ...
                R_DATA: begin
                    // Present read data
                    if (read_addr_valid && !read_boundary_cross) begin
                        axiIF.RDATA <= mem_rdata;
                        axiIF.RRESP <= 2'b00;  // OKAY
                    end else begin
                        axiIF.RDATA <= {DATA_WIDTH{1'b0}};
                        axiIF.RRESP <= 2'b10;  // SLVERR
                    end
                    
                    axiIF.RVALID <= 1'b1;
                    axiIF.RLAST <= (read_burst_cnt == 0);
